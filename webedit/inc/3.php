...
Short-Get-Token: <input type="checkbox" onchange="var re=xhr(3,{a:1,i:this.checked?1:0});if(re.charAt(0)!='['){alert('Error');this.checked=!this.checked;}else{re='?sgt='+JSON.parse(re)[0];$('sgtBkmk').href=re;$('sgtBkmk').innerHTML='<?=$_SERVER['HTTP_HOST'].$dir?>'+re;$('sgtLink').value='<?=$_SERVER['HTTP_HOST'].$dir?>'+re;}$('sgt').style.display=this.checked?'block':'none';"<?=$sgt==''?'':' checked'?>>&emsp;<a class="help" onclick="alert('With the Short-Get-Token you can log in automatically. Save the link below as a bookmark. To sign in, simply open the bookmark.');">?</a>

<div id="sgt"<?=$sgt==''?' style="display:none;"':''?>><code><a id="sgtBkmk" href="<?=$dir.'?sgt='.$sgt?>" onclick="return false;" draggable="true" style="cursor:move;"><?=$_SERVER['HTTP_HOST'].$dir.'?sgt='.$sgt?></a></code>
<input id="sgtLink" type="text" style="display:none;" value="<?=$_SERVER['HTTP_HOST'].$dir.'?sgt='.$sgt?>" disabled><svg width="15" height="20" title="Copy" style="margin-left:1em;transform:translateY(.4em)" onclick="$('sgtLink').style.display='initial';$('sgtLink').select();alert(document.execCommand('copy')?'Link copied':'Error');$('sgtLink').style.display='none';"><style>rect{fill:#<?=$d?'FFF':'333'?>;stroke:#<?=$d?'000':'DDD'?>;stroke-width:2px;width:9px;height:14px;}</style><rect x="1" y="1" rx="2" ry="2" /><rect x="5" y="5" rx="2" ry="2" /></svg> <span style="cursor:pointer" title="Renew" onclick="var re=xhr(3,{a:1,i:1});if(re.charAt(0)!='['){alert('Error');}else{re='<?=$_SERVER['HTTP_HOST'].$dir?>?sgt='+JSON.parse(re)[0];$('sgtBkmk').href=re;$('sgtBkmk').innerHTML=re;$('sgtLink').value=re;}">↻</span></div>
</div></details><hr>
